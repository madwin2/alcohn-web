...
export default function ContactoPageContent({ market }: ContactoPageContentProps) {
  const breadcrumbJsonLd = marketBreadcrumbJsonLd(market, [
    { name: 'Inicio', path: '/' },
    { name: 'Contacto', path: '/contacto' },
  ]);
  const buyCta = marketBuyCta(market, 'Subir logo y ver precio');
  const secondaryCatalogCta = marketStandardSecondaryCta(market);
            ...
            <section className="technical-sheet hidden p-6 md:block md:p-8">
              <div className="relative z-10">
                ...
                <p className="text-sm leading-relaxed text-neutral-700 mb-6">
                  Si tenes el logo y queres ver como quedaria podes usar el diseñador online.
                  {market === 'ar'
                    ? ' Para diseños estandar, podes entrar a sellos estándar.'
                    : ' Para ver opciones disponibles, entrá al catálogo.'}{' '}
                  Para dudas técnicas, revisá FAQ.
                </p>
                <div className="flex flex-col gap-3">
                  <ActionButton href={marketBuyPath(market)} variant="primary">
                    Subir logo y ver precio
                  </ActionButton>
                  <ActionButton href={secondaryCatalogCta.href} variant="secondary">
                    {secondaryCatalogCta.label}
                  </ActionButton>
                  <ActionButton href={marketPath(market, '/faq')} variant="ghost">
